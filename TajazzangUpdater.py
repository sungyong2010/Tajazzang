...
REMOTE_VERSION_URL = "https://raw.githubusercontent.com/sungyong2010/anything-to-share/main/Tajazzang/version.json"
REMOTE_EXE_URL = "https://raw.githubusercontent.com/sungyong2010/anything-to-share/main/Tajazzang/Tajazzang.exe"
TARGET_EXE_PATH = r"c:\Apps\tajazzang\Tajazzang.exe"
LOCAL_VERSION_JSON = r"c:\Apps\tajazzang\version.json"  # Store local version for comparison
BACKUP_DIR = r"c:\Apps\tajazzang\backup"
LOG_PATH = r"c:\temp\tajazzangupdater.log"
SCHEDULED_TASK_NAME = r"\Tajazzang 자동 실행"
...
def run_interactive_command(command: str) -> bool:
    ...
    task_name = f"TajazzangTemp_{int(time.time())}"
    try:
...
def show_popup(message: str, title: str = "Tajazzang Updater", timeout_ms: int = 2500, flags: int = 0x00000040):
...
def terminate_running_exe(exe_path: str, process_name: str = "tajazzang.exe", timeout: int = 10):
    logging.info("Checking for running tajazzang.exe processes to terminate")
    ...
    while time.time() < end_time:
        ...
        for proc in psutil.process_iter(['pid', 'name', 'exe']):
            try:
                ...
                pass
        if not alive:
            logging.info("All tajazzang.exe processes terminated")
            return True
        time.sleep(0.5)
    logging.error("Timeout waiting for tajazzang.exe to terminate")
    return False
...
def backup_existing(exe_path: str):
    ...
    dest = os.path.join(BACKUP_DIR, f"tajazzang_{timestamp}.exe")
    try:
...
def download_new_exe(url: str) -> str:
    logging.info(f"Downloading new tajazzang.exe from {url}")
    try:
        resp = requests.get(url, timeout=30)
        resp.raise_for_status()
        temp_dir = tempfile.gettempdir()
        temp_path = os.path.join(temp_dir, "tajazzang_new.exe")
